...
node tools/cdp-type-click.mjs <qa_boot-url> [port] [stepRegex]      # 【F】1 的"真实点击走一遍交互"：14 步走完 菜单→启动→HUD→传送→跃迁→
                                                                   # 静音→拍照→计时赛→排行榜→关闭。每步先 `document.elementFromPoint` 命中测试
                                                                   # 再发 `Input.dispatchMouseEvent`（`.click()` 看不见压在上面的层），
                                                                   # 之后**原地重跑一遍 hud-audit-probe**（尺子只有一把，代价每步 ~60 s），
                                                                   # 所以菜单/传送面板/排行榜用的是同一把字号·字距·溢出·对比度尺子；
                                                                   # 判绿＝末行 `STEPS 14 FAILED 0` 且每个 measure 态 min=12 / h=v=col=0 / 三 control 全绿；
