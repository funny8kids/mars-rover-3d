...
node tools/cdp-type-click.mjs <qa_boot-url> [port] [stepRegex]      # 【F】1 的"真实点击走一遍交互"：16 步走完 菜单→启动→HUD→动效落位（含
                                                                   ...
                                                                   # 字号·字距·溢出·对比度尺子；
                                                                   # 判绿＝末行 `STEPS 16 FAILED 0` 且每个 measure 态 min=12 / h=v=col=0 / 三 control 全绿；
