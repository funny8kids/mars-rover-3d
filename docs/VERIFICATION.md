...
node tools/ref-site-font-probe.mjs                                 # 规格侧对照尺：同一支函数跑 bruno-simon.com 线上 CSS 与 src/styles.css
                                                                   ...
                                                                   # §4 之后它先剥掉 CSS 注释再数动效（注释里写一句 cubic-bezier 不算做过动效），
                                                                   # 并且把 var(--ease-*) 展开后再判「有没有被用上」：
                                                                   # `bareTransitions` / `beziersApplied` / `reducedMotionTokens`
node tools/cdp-run.mjs <url|-> tools/motion-landing-probe.js       # 动效**落位**尺：读 getComputedStyle，量层叠之后真正用来插值的曲线；
                                                                   # landed 8/8 必须是越界 cubic-bezier，连续读数（电力/积尘/推进条）必须不是，
                                                                   # 胶囊的进入是 @keyframes（display:none 起步，transition 插不出来）；
                                                                   # 第二遍由 cdp-type-click 用 Emulation.setEmulatedMedia 打开 reduce，同一批必须 0/8
node tools/cdp-camera-hold.mjs <url> <probe-file>                  # §6 的两帧对照：同一台车在 0 与 ~47 km/h 各拍一张（`__RSB.shot`，
                                                                   # 相机交给游戏自己摆），报两张的亮度直方图与 clip（判据 clip=0、最暗档为 0）
node tools/cdp-type-click.mjs <qa_boot-url> [port] [stepRegex]      # 【F】1 的"真实点击走一遍交互"：24 步走完 菜单→启动→HUD 任务日志七手势→动效落位（含
                                                                   # reduce 第二遍）→传送→跃迁→静音→拍照→计时赛→排行榜→关闭。每步先
                                                                   # `document.elementFromPoint` 命中测试再发 `Input.dispatchMouseEvent`
                                                                   # （`.click()` 看不见压在上面的层），之后**原地重跑一遍 hud-audit-probe**
                                                                   # （尺子只有一把，代价每步 ~60 s），所以菜单/传送面板/排行榜用的是同一把
                                                                   # 字号·字距·溢出·对比度尺子；
                                                                   # 判绿＝末行 `STEPS 24 FAILED 0` 且每个 measure 态 min=12 / h=v=col=0 / 三 control 全绿；
                                                                   # 用 stepRegex 跑子集时必须把 `menu:start` 留在里面，否则游戏没启动、HUD 那几步会红成"phase never arrived"；
                                                                   # 探针回 {fail}（phase 没等到）或没有 type.min 时该步判红，不静默算过
...
| 前端 bruno-simon 差距 | 结论与七条「现象 / 实测 / 对照事实 / 修法 / 判负标准」在 `docs/FRONTEND_BRUNO_SIMON_GAP.md`；这一行只登记尺子与红件。三把尺子：`node tools/cdp-run.mjs "http://127.0.0.1:5173/qa_boot.html?auto=std&audstate=hud" tools/hud-audit-probe.js 9333 90000 300000`（量**真渲染出来**的东西：DOM 字号/字距/覆盖/对比度普查 + 相机投影算术，~60 s）、同一命令换 `&audstate=drive`（沿真实加速过程逐档采镜头构图，~150 s；分两趟是因为一次 `Runtime.evaluate` 只有一个帧预算，约 500 档步进帧就让 CDP 回 `-32603`）、`node tools/ref-site-font-probe.mjs`（同一支函数跑参考站线上 CSS 与我们的 `src/styles.css`，量**作者写下的规格**）。实测：`document.fonts` 0 个 face，canvas 推进宽度反证 `--mono` 落到 DejaVu Sans Mono（657.97 px）、`--sans`/`.title` 落到 Noto Sans CJK SC（607.64 px）——即 `getComputedStyle().fontFamily` 回的名字全是自证（参考站 3 个 @font-face + Amatic SC 700 + Nunito `display=block`）；屏上阶梯 `{10:2,11:7,12:1,12.5:4,14:1,52:1}`（min 10 px、`under11 12.5 %`、`tracked15 43.8 %`、`textCoveragePct 9.1`、4 个盒子 `10.01 %`）；动效 15 条 transition / 5 组 keyframes / **0 条 cubic-bezier**、7 条 text-shadow（参考站 23 / 1 / 4 条 bezier 全部过冲 / 0）。**闸门见过红（M1）**：把 `src/camera/chase.js:74` 的 `speed * 0.42` 改成 `0`（锚点全仓唯一），drive 档的 14 帧 fov 阶梯**全部躺平在 60.0**、占比仍从 12.41 掉到 8.52 —— 说明这一档量的确实是游戏相机，而主体变小另有其因；还原后 `git diff` 空。修正后的归因由 `shareAttribution` 给出：加速时主体高度占比 −38.1 %，其中**后退 −31.5 %、开镜 −9.9 %**（`factorsInto 0.617` 对上实测比值 `opticsCheck 0.619`，`rulerControl` 两把独立尺 9.94 == 9.94），所以「fov 变宽被后退盖过」这种说法是错的，两条都在把车缩小；判负标准 `totalPctLost ≤ 15` 且 `top.horizonPct − atRest.horizonPct ≤ 2`。已证伪、不要重提的七条（含「对比度不够」「阶梯不够拉」「静态 fov 60° 是错的」）同样列在那份文档的表里；**§2/§3 已于 2026-09-24 落地**：字号地板抬到 12 px、宽字距退回 display 层之后，hud 档阶梯 `{12:8,13:5,15:2,52:1}`（`type.min 12`、`under11 0`、`tracked15 0`），规格侧 `fontSizes[0] = 12px`；探针新增 `menu` phase（`window.__AUDSTATE` / `audstate=menu`）后菜单档 31 个节点 `{12:27,18:1,20:1,22:2}`、`tracked15 12.9 %`（四条全部 ≥18 px），八态 `contrast.below45 = 0`（最暗 `.q-card i`：en 4.72 / zh 4.92，hud 档 min 5.5）。`tools/cdp-type-click.mjs` 的真实点击走查抓到两件 DOM 读数看不见的事：`#lang-btn`（z-index 40）被 `.overlay`（50）压在下面，菜单那一屏点不动——历次语言测试全绿是因为它们调 `.click()`；`kbd` 提到 12 px 后 `#controls-hint` 一排键帽悬出本行 2 px（`v.n = 10`）。两处修完复测 `STEPS 14 FAILED 0`、每态 `h=0 v=0 collisions=0`、三个 control 全绿。抓帧右侧那条黑带是 qa_boot 把渲染器尺寸改成 889×967 而 DOM 仍是 1280×713 造成的（`frame.laid` vs `frame.canvas`），不是游戏里的死黑；**§4 亦已落地**：`:root` 四条全过冲曲线（pop/glide/lean/exit）+ 七处裸 `transition:.Ns` 清零 —— 规格侧 `transitions 15→20、bare 0、beziers 4、overshoot 4、applied 4`（参考站 23/2/4/4/4），渲染侧 `motion-landing` landed 8/8、连续读数 3/3 保持 linear、胶囊进入 2/2 走 `@keyframes`，`Emulation.setEmulatedMedia` 打开 reduce 后同一批 landed 8→0、四个 token 解析成 `linear`、进入时长 `1e-05s`；RED 对照＝同一把新尺子跑改动前的 styles.css 得 `bare 7 / beziers 0 / applied 0 / reducedTokens []`。踩到两条：给 `li.active` 加 3 px 侧倾后 `#mission-list 326>323` 报溢出，把补位放在 `#mission-panel` 的 padding 上**无效**（自适应盒子的外侧 padding 只长 border box），放进列表自己的 `padding-right` 才归零；规格尺子原本 `Object.fromEntries` 建 var 表 ⇒ reduce 块里最后写的 `linear` 覆盖真曲线、`applied` 假红，改为取第一条定义并把覆盖另报成 `reducedMotionTokens`。**§5 亦已落地**：`textShadows 7 → 2`，留下的两处是画面里真有光源照字的对象（`#countdown` 点火 T- 秒、`#tel-wrap` 尾羽下的发射板），其余交给字号/字重/色阶（`.logo-glyph`、`#speed-val` 去掉光晕，`li.active` 换 600 字重 + 3 px 侧倾），两处深色墨影（`#nav-chip`、`.tel-ev`）改由各自底板承担（胶囊 bg `.74→.86`、日志加 `rgba(9,11,17,.55)` 小底板），并撤掉 `#mission-list li` 上挂在不变性质里的 `text-shadow` 过渡；去发光后八个状态 `contrast.below45 = 0`（hud `min 5.5`、菜单 `4.72–4.73`）、`h=0 v=0 collisions=0`、`STEPS 16 FAILED 0`。**§6 亦已落地**：drive 档 `totalPctLost 38.1 → 1.8`、`horizon Δ 6.3 → −0.5`（阶梯全程 fps 60）——`chase.js` 让 `back` 与开角联动，再把跟随滤波器的稳态拖尾 `lag = speed/stiff` 付回去（新增 `plan` 读数证实 achieved dist ≡ back + lag，48 km/h 拖 3.96 m），俯角由 `up = 1.5 + (hold+la)·0.1844` 导出，相机避让保留半径从一律 `r+1.8` 改为 `clamp(r·1.2,.35,1.8)`（一根 0.45 m 灯杆曾把机位顶开 3 m：dist 11.8→16.2、占比 12.5→7.9）；尺子自己也修了一处——drive 档 `atRest` 原来只等 40 帧，量到的是仍在飞入的相机（dist 12.8 对落定后 10.3），现在 step 到不动为止并报 `atRestFrames`。两把自证仍绿（`rulerControl` 13.66==13.66、`opticsCheck` 0.982 vs 0.978）；`tools/cdp-camera-hold.mjs` 另拍 0 / 47 km/h 两帧：`clip=0`、最暗档 0 像素、两张直方图同形。**§7 亦已落地（2026-09-24）**：走查从 16 步长到 24 步，其中七步是任务日志的手势不变量 `hud:mission-state` —— 它要求 `#mp-more` 在开/收两态下的**锚点 (x, y) 只有一个值**且任一按下点落在其余矩形之内（宽度不参与判定，因为「还有 3 条」与「收起任务」本来就两个长度）。这条不变量抓到「控件在指针底下跑掉」的两个实例（计数器压在四条被展开的句子下面 y 58→160；收起态顺手 `display:none` 掉按钮上方的 `.mp-title` y 58→85），细节与 RED 件在 `docs/FRONTEND_BRUNO_SIMON_GAP.md` §7。**§7 的判负标准也已落成一道会拒绝执行的闸门**：`tools/hud-audit-probe.js` 的 `density` 块（`bar {textPct:5, panelPct:5, cardedBoxPct:3}` ＋ 反装饰的 `control {bar:1, tripped:3}`）由 `tools/cdp-type-click.mjs` 消费，超线即 `!! … FAIL §7 density: …`；判的是"玩家平时盯着的那一屏"，玩家亲手放上去的层（展开的日志、排行榜、菜单）记成 `pinned` / `summonedUp` 后不进闸门。走查因此长到 25 步（多的一步 `hud:pad-leave` 验的是**条件式控件的另一半**：藏起来的提示退场后传送胶囊必须回来，靠按住 W 让车真的开下光台）。这道闸门**先见过红**（停在光台上盒子 5.46 %，五条一起红：`/tmp/j7-gate-full.txt`），修在产品里（同屏两句同样的召唤去掉了重复的那句，没有改 `BAR` 也没有加豁免），最终字节那一跑以末行 `FAILED 0` 为准（`/tmp/j9-full.txt`），逐屏读数与两者对照表在 GAP §7 末尾的实跑记录。闸门自己也做过极性验证：`#mission-panel` padding 10→40 px（锚点全仓唯一）后它如实判负 `MUTATION CAUGHT`，备份 `cp -a` 还原、`sha256sum` 回到原值——顺带抓到还原判定自己的一条假红（`sha256sum < file` 输出 `hash  -`，永远不等于裸 hash），改 `| cut -d' ' -f1` 后干净/做肥两个极性都验过 |
...
重跑产物用的第三把尺**还没有**接到写侧：`python3 tools/glb_repro_pair.py <glb 路径> -- <重跑命令>` 会连跑
两次同一份 builder 并按差在哪一段命名退出码（`0` 逐字节相同 / `1` 索引顺序漂 / `3` 只剩 float 漂）。
它没进上面那条链，是因为 `launch_tower` 在最好的情况下仍停在 `3`（`bmesh.ops.bevel(clamp_overlap=True)` 的
66 字节 UV 漂，`canon_polys` 钉不住，见 `tools/logs/repro-uv-sphere-2026-09-27.log` §2）——先把尺子与红件
登记在这里，等 float 漂归零再让它也拒绝执行；现在写死就是拿一把已知会红的闸当装饰。
