...
def emit_js(report):
    """Write the collision table `src/world/rim_rock.js` is generated from, then
    read it back against the build report.

    Exit-code contract (audit 2026-09-26, task #95/4): this post-export step used
    to traceback with FileNotFoundError in environments that ship the tools
    without the src tree — the asset itself was fine, and blender --background
    still left the caller at rc=0, so the crash and the success were
    indistinguishable from the exit code alone. Absence is now a named skip that
    exits 0; a present source tree that cannot be written or that reads back
    wrong exits non-zero. The two outcomes are told apart by the RIMROCK_VERIFY_*
    line, never by silence. Returns "skipped" or "verified" so the RIMROCK_DONE
    line can state which of the two the run actually did.
    """
    src_dir = os.path.dirname(SRC_JS)
    if not os.path.isdir(src_dir):
        print("RIMROCK_VERIFY_SKIPPED 跳过校验，因为 %s 不存在（本环境没有 src 树："
              "缺席不是失败；碰撞表无需重写，GLB 本身已在 build_kit 的 verify 里"
              "按几何校验过）" % SRC_JS)
        return "skipped"
    ...
    try:
        ...
        # Every named line goes to stdout, then the exit code — `SystemExit("msg")` puts the
        # message on stderr, where a caller grepping the build log for the counterpart of
        # RIMROCK_VERIFY_OK sees nothing at all.
        print("RIMROCK_VERIFY_FAILED 无法写入 %s: %s" % (SRC_JS, e))
        raise SystemExit(1)
    ...
    try:
        ...
        print("RIMROCK_VERIFY_FAILED 写后无法读回 %s: %s" % (SRC_JS, e))
        raise SystemExit(1)
    gone = [ln for ln in lines if ln.strip() and ln not in back]
    if gone:
        print("RIMROCK_VERIFY_FAILED %s 与本次构建报告不一致，缺 %d 行，例如："
              % (SRC_JS, len(gone)))
        for ln in gone[:5]:
            print("   ", ln)
        raise SystemExit(1)
    print("RIMROCK_VERIFY_OK %s（%d clasts, %d discs 全部读回一致）"
          % (SRC_JS, len(report), sum(len(d["discs"]) for d in report.values())))
    return "verified"
...
def build_kit(spread=0.0):
    ...
    for (i, (name, tall, cuts, seed, discs, foot, shape, crest)) in enumerate(NODES):
        ...
        if max(bulge_mm, gap_mm) > TOL_MM:
            print("RIMROCK_FIT %s bulge=%dmm gap=%dmm over %dmm tolerance"
...
if __name__ == "__main__":
    ...
    #
    # Exit-code contract (audit 2026-09-26, task #95/4): rsbkit.export() ignores the gltf
    # operator's return status, and an uncaught exception here would leave blender --background
    # at rc=0 with no RIMROCK_DONE line — indistinguishable from a green build to anything
    # reading the exit code. So every stage reports into a named line and exits non-zero itself:
    #   RIMROCK_FIT            a clast's mesh disagrees with its authored discs (>TOL_MM)
    #   RIMROCK_FAILED         a stage raised (builder, verify, exporter's own size read)
    #   RIMROCK_EXPORT_FAILED  the asset on disk is missing, empty or older than this run
    #   RIMROCK_VERIFY_FAILED  the collision table could not be written or read back
    #   RIMROCK_VERIFY_SKIPPED no src tree here: not a failure, said out loud instead of by
    #                          silence (build_kit's geometric verify still had to pass)
    #   RIMROCK_ABORT          RIMROCK_SPREAD is set but is not a number
    #                          (measured: `RIMROCK_SPREAD=banana` raised a ValueError inside the
    #                          env read, which left blender at rc=0 with a traceback and no line)
    import time, traceback
    raw_spread = os.environ.get("RIMROCK_SPREAD", "0")
    try:
        spread = float(raw_spread)
    except ValueError:
        print("RIMROCK_ABORT RIMROCK_SPREAD=%r is not a number" % raw_spread)
        raise SystemExit(1)
    fname = "rim_rock_preview.glb" if spread else "rim_rock.glb"
    # Two seconds of slack: some filesystems stamp mtimes to the whole second, and a
    # build that starts at hh:mm:59.9 would otherwise call its own export stale.
    # A leftover from an earlier run is minutes to days old, so the check still bites.
    ...
    try:
        ...
        traceback.print_exc()
        print("RIMROCK_FAILED asset=%s stage raised — nothing downstream is trusted" % fname)
        raise SystemExit(1)
    # The asset has to be on disk, non-empty and written by this run: a leftover GLB from an
    # earlier build would otherwise let a failed export print a success line.
    path = os.path.join(OUT, fname)
    try:
        fresh = os.path.getmtime(path) >= t_start
        size = os.path.getsize(path)
    except OSError as e:
        print("RIMROCK_EXPORT_FAILED %s unreadable: %s" % (path, e))
        raise SystemExit(1)
    if not fresh or size <= 0:
        print("RIMROCK_EXPORT_FAILED %s bytes=%d fresh=%s (not written by this run)"
              % (path, size, fresh))
        raise SystemExit(1)
    table = "preview mode, not rewritten (RIMROCK_SPREAD)"
    if not spread:
        status = emit_js(report)
        if status == "verified":
            table = "rewritten and read back: %s" % SRC_JS
        elif status == "skipped":
            table = "NOT rewritten — no src tree, see RIMROCK_VERIFY_SKIPPED"
        else:
            # A status neither branch can mean: fail loudly rather than let a
            # KeyError unwind into blender's rc=0.
            print("RIMROCK_VERIFY_FAILED emit_js returned %r, no such outcome" % status)
            raise SystemExit(1)
    print("RIMROCK_DONE %s bytes=%d (%d clasts, collision table: %s)"
          % (fname, size, len(report), table))
